...
datas = []
f_r = open('../data/v1.5.0推理结果.json', 'r', encoding='utf-8')
f = open('../data/测试集-车辆属性-场景-v3.5-有运输车.json', 'r', encoding='utf-8')
...
for i in range(len(lines_r)):
    ...
    direction_r = line_r_str['label'][0]['data'][0]['class'][0]
    score = line_r_str['label'][0]['data'][0]['scores'][0]
    if direction == "":
        continue
    if score >= 0.7:
...
# data_tj.to_csv('./data/v1.5.0统计-方向.csv')
