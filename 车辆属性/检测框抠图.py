...
class GetPicItems(threading.Thread):
    def run(self):
        while (len(lines) > 0):
            ...
            url = line_dict['url']
            photo = url.split('/')[-1]
            file = '/Users/smai/Desktop/work/smmc-ks/clsx1/' + photo
            ...
            for index in range(0, len(box_list)):
                ...
                imglist = os.listdir('/Users/smai/Desktop/plate/clsx-kt1/')
...
if __name__ == '__main__':
    # 文件地址传参
    photos = os.listdir('/Users/smai/Desktop/work/smmc-ks/clsx1/')
    r_path = r'data05/TASK_测试集-车辆属性-车型-特种车辆检测_3044.json'
    # dstpath路径不能带中文的,原因就是cv2.imwrite，入参路径不能带中文的
    r_f = open(r_path, 'r', encoding='utf-8')
    lines = r_f.readlines()
    saveBasepath = '/Users/smai/Desktop/plate/clsx-kt1/'
    for x in range(20):
        g = GetPicItems()
        g.start()
    for x in range(10):
